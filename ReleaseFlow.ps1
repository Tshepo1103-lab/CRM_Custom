...
$mainVersion = "0.0.0" + "-build$PipeBuildId"
...
if ("$PipeSourceBranch" -like "*/pull/*" -and "$PipeTargetBranch" -ne ''){
...
if ("$PipeSourceBranch" -like "*/main" -or "$PipeSourceBranch" -like "*/merge"){
   write-host "Source Branch should be main or merge here: $PipeSourceBranch"
   write-host "##vso[task.setvariable variable=currentBranch]$PipeSourceBranch"
   write-host "##vso[task.setvariable variable=versionNo]$mainVersion"
 }